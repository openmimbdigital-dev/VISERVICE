...
class ReopenEventParticipationAction
{
    use AsAction;
    ...
    public function handle(int $event_id): Event
    {
        ...
        abort_unless($event->participation_closed, 422, 'La toma de participación no está cerrada.');

        $event->update(['participation_closed' => false]);
        $event = $event->fresh();

        LogUserHistoricalAction::run(
            action: 'reopened',
            module: 'events.participation',
            description: "Abrió la toma de participación del evento {$event->name}",
            subject: $event,
