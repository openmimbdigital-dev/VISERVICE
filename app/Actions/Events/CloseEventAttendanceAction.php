...
class CloseEventAttendanceAction
{
    use AsAction;
    ...
    public function handle(int $event_id): Event
    {
        ...
        abort_unless(! $event->attendance_closed, 422, 'La toma de asistencia ya está cerrada.');

        $event->update(['attendance_closed' => true]);
        $event = $event->fresh(['attendee_types']);

        LogUserHistoricalAction::run(
            action: 'closed',
            module: 'events.attendance',
            description: "Cerró la toma de asistencia del evento {$event->name}",
            subject: $event,
