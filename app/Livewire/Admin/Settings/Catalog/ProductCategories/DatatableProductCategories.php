...
class DatatableProductCategories extends LivewireDatatable
{
    ...
    public function onProductCategoryDeleted(): void {}

    #[On('product-category-saved')]
    public function onProductCategorySaved(): void {}

    public function builder(): Builder
    {
        $products_count = DB::table('products')
            ->select('product_category_id', DB::raw('COUNT(*) as products_count'))
        ...
        $products_count->groupBy('product_category_id');
        ...
        $query = ProductCategory::query()
            ...
            ->leftJoinSub(
                $products_count,
                'product_usage',
                fn ($join) => $join->on('product_categories.id', '=', 'product_usage.product_category_id')
            )
            ->addSelect('product_usage.products_count')
            ->orderByDesc('product_categories.created_at');

        if (auth()->user()->hasRole('superAdmin')) {
            $query->leftJoin('businesses', 'product_categories.business_id', '=', 'businesses.id');
        }
    ...
    {
        $columns = [
            Column::name('product_categories.name')
                ->label('Nombre')
                ->searchable()
                ->sortable(),

            Column::callback(['product_categories.inventory'], function ($inventory) {
                $label = $inventory ? 'Sí' : 'No';

                return '<span class="text-sm text-slate-700">' . $label . '</span>';
            })->label('Inventario')->filterable([1 => 'Sí', 0 => 'No']),

            Column::callback(['product_categories.general'], function ($general) {
                if ($general) {
        ...
        return array_merge($columns, [
            Column::callback(['product_categories.active'], function ($active) {
            ...
            Column::callback(
                ['product_categories.id', 'product_categories.general', 'product_categories.business_id', 'product_usage.products_count'],
                function ($id, $general, $business_id, $products_count) {
                    $permissions = $this->catalogRowPermissions(
                        (bool) $general,
                        $business_id,
                        (int) $products_count,
                        'settings.product_categories.edit',
                        'settings.product_categories.delete',
                    );

                    return view('livewire.admin.settings.catalog.product-categories.actions', [
                        'id'                  => $id,
    ...
    {
        $this->dispatch('open-product-category-edit', id: $id);
    }
    ...
    protected function onDeleteConfirmed(): void
    {
        try {
            ...
            $this->dispatch('product-category-deleted');
        } catch (\Throwable) {
