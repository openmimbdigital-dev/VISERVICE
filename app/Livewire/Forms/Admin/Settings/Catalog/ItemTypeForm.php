...
class ItemTypeForm extends Form
{
    ...
    public function setItemType(ItemType $item_type): void
    {
        $this->item_type_id = $item_type->id;
        $this->name         = $item_type->name;
        $this->active       = $item_type->active;
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'name' => [
                ...
                'string',
                'max:100',
                new NotConflictingWithGeneralCatalogName(ItemType::class, $this->item_type_id),
                Rule::unique('item_types', 'name')->where($scope)->ignore($this->item_type_id),
                function (string $attribute, mixed $value, \Closure $fail) use ($scope) {
                    ...
                    $query = ItemType::query()->whereRaw('LOWER(TRIM(name)) = ?', [$name]);
                    $scope($query);

                    if ($this->item_type_id) {
                        $query->where('id', '!=', $this->item_type_id);
                    }
                    ...
                    $query = ItemType::query()->where('label', $label);
                    $scope($query);
