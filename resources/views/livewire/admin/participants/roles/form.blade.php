<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">
                    {{ $form->isEditing() ? 'Editar rol de participante' : 'Nuevo rol de participante' }}
                </h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Define el nombre y la descripción del rol.</p>
            </div>
            <a href="{{ route('admin.participants.roles.index') }}" wire:navigate class="btn btn-outline-secondary btn-sm w-full justify-center sm:w-auto">Volver</a>
        </div>
    </header>
    ...
    <form wire:submit="save" class="space-y-6">
            ...
            <div class="space-y-5 p-4 sm:p-5">
                @if($is_super_admin)
                    <div>
                        ...
                        <select wire:model="form.business_id"
                            class="form-select w-full border px-3.5 py-2.5 text-sm @error('form.business_id') border-rose-400 bg-rose-50 @enderror">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input wire:model="form.name" type="text" placeholder="Ej. Voluntario, Líder, Invitado"
                            class="form-input w-full border px-3.5 py-2.5 text-sm @error('form.name') border-rose-400 bg-rose-50 @enderror">
                ...
                <div>
                    ...
                    <textarea wire:model="form.description" rows="4" placeholder="Describe el propósito de este rol"
                        class="form-input w-full border px-3.5 py-2.5 text-sm @error('form.description') border-rose-400 bg-rose-50 @enderror"></textarea>
        ...
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <a href="{{ route('admin.participants.roles.index') }}" wire:navigate class="btn btn-outline-secondary w-full justify-center sm:w-auto">Cancelar</a>
