<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Roles de participantes</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Catálogo de roles que puedes asignar a los participantes del negocio.</p>
            </div>
            <div class="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row">
                <a href="{{ route('admin.participants.index') }}" wire:navigate class="btn btn-outline-secondary btn-sm w-full justify-center sm:w-auto">
                    Volver a participantes
                </a>
                @can('participants.roles.create')
                <x-ui.create-button :href="route('admin.participants.roles.create')" class="w-full justify-center sm:w-auto">
                    Nuevo rol
                </x-ui.create-button>
                @endcan
            </div>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        ...
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:admin.participants.roles.datatable-participant-roles :key="'participant-roles-datatable'" />
        </div>
    </section>
</div>
