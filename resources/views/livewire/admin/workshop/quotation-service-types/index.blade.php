<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="text-slate-300">/</span>
        <span class="font-semibold text-slate-900">Tipos de servicio</span>
    </nav>
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Tipos de servicio</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Catálogo de tipos de servicio para cotizaciones (mantenimiento preventivo, correctivo, etc.).</p>
            </div>
            @can('workshop.quotation_service_types.create')
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        @foreach([['label'=>'Total','value'=>$stats['total']],['label'=>'Activos','value'=>$stats['active']],['label'=>'Generales','value'=>$stats['general']]] as $stat)
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        ...
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:admin.workshop.quotation-service-types.datatable-quotation-service-types :key="'quotation-service-types-datatable'" />
        </div>
    </section>
    ...
    <x-ui.modal centered maxWidth="md">
        ...
        <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6">
            <h3 class="text-base font-semibold text-slate-900">
                {{ $form->isEditing() ? 'Editar tipo de servicio' : 'Nuevo tipo de servicio' }}
            </h3>
