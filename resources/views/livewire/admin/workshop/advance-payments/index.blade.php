<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <p class="mt-2 max-w-xl text-sm text-slate-600">Seguimiento de anticipos y abonos registrados en órdenes de trabajo.</p>
            </div>
        </div>
    </header>
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div class="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm ring-1 ring-slate-900/[0.035]">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Confirmados</p>
            <p class="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{{ $stats['confirmed'] }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm ring-1 ring-slate-900/[0.035]">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Anulados</p>
            <p class="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{{ $stats['voided'] }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm ring-1 ring-slate-900/[0.035]">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Total confirmado</p>
