<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="text-slate-300">/</span>
        <span class="font-semibold text-slate-900">Documentos asociados</span>
    </nav>
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Documentos asociados OT</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Catálogo de documentos asociados a órdenes de trabajo para tu negocio.</p>
            </div>
            @can('workshop.work-orders.associated-documents.create')
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:max-w-sm">
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        ...
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:admin.workshop.work-orders.associated-documents.datatable-associated-documents :key="'associated-documents-ot-datatable'" />
        </div>
    </section>
    ...
    <x-ui.modal centered maxWidth="md">
        ...
        <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6">
            <h3 class="text-base font-semibold text-slate-900">
                {{ $form->isEditing() ? 'Editar documento asociado' : 'Nuevo documento asociado' }}
            </h3>
        ...
        <form wire:submit="save" class="flex min-h-0 flex-1 flex-col">
            ...
            <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 px-4 py-4 sm:flex-row sm:justify-end sm:gap-3 sm:px-6">
                ...
                <button type="submit" wire:loading.attr="disabled" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-60 sm:w-auto">
                    <span wire:loading.remove wire:target="save">{{ $form->isEditing() ? 'Guardar cambios' : 'Crear documento' }}</span>
