<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
            ...
            <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-2">
                <div class="relative">
                    ...
                    <select wire:model="form.item_type_id" class="form-select w-full border px-3 py-2 text-sm">
                        <option value="">Seleccionar tipo</option>
                        @foreach($item_types as $item_type)
                            <option value="{{ $item_type->id }}">{{ $item_type->name }}@if(! $item_type->active) (inactivo)@endif</option>
                        @endforeach
                    </select>
                    @error('form.item_type_id')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                </div>

                <div class="relative">
                    <label class="label-up">Categoría <span class="text-rose-500">*</span></label>
                    <select wire:model.live="form.item_category_id" class="form-select w-full border px-3 py-2 text-sm">
                        <option value="">Seleccionar categoría</option>
                        @foreach($item_categories as $item_category)
                            <option value="{{ $item_category->id }}">{{ $item_category->name }}@if(! $item_category->active) (inactiva)@endif</option>
                        @endforeach
                    </select>
                    @error('form.item_category_id')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                    @php $selected_category = $item_categories->firstWhere('id', (int) $form->item_category_id); @endphp
        ...
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3">
            <a href="{{ route('admin.catalog.items.index') }}" wire:navigate class="btn btn-outline-secondary">Cancelar</a>
