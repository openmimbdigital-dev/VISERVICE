<div class="flex flex-wrap justify-end gap-1">
    <a href="{{ route('admin.events.manage.category.show', [$event_category_id, $id]) }}" wire:navigate title="Ver detalle"
    ...
    @if($can_edit)
        @if(! empty($edit_disabled))
            <span title="{{ $edit_disabled_title ?? 'No se puede editar este registro' }}"
        ...
        @else
            <a href="{{ route('admin.events.manage.category.edit', [$event_category_id, $id]) }}" wire:navigate title="Editar"
                class="rounded p-1.5 text-slate-400 transition-colors hover:bg-indigo-50 hover:text-indigo-600">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
    ...
    @if($can_delete)
        @if(! empty($delete_disabled))
            <span title="{{ $delete_disabled_title ?? 'No se puede eliminar este registro' }}"
                class="cursor-not-allowed rounded p-1.5 text-slate-300 opacity-50"
                aria-disabled="true">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
