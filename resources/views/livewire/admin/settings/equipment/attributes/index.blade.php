<div class="relative mx-auto w-full max-w-[90rem]">
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex w-full shrink-0 flex-wrap gap-2 sm:w-auto">
                ...
                @can('settings.attributes.create')
                <x-ui.create-button
                    :href="route('admin.settings.equipment.attributes.create')"
                    size="sm"
                    class="flex-1 sm:flex-none justify-center"
                >
                    {{ $config['create_button_text'] ?? 'Nuevo atributo' }}
                </x-ui.create-button>
                @endcan
            </div>
        </div>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-4 sm:px-5">
            <h2 class="font-semibold text-slate-800">Listado de atributos</h2>
        </div>
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:dynamic-component :component="$config['datatable_component']" :key="'attributes-datatable'" />
        </div>
    </section>
</div>
