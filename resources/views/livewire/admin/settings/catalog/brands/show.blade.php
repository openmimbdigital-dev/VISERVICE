    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex w-full shrink-0 flex-wrap gap-2 sm:w-auto">
                <a href="{{ route('admin.settings.catalog-products.brands.index') }}" wire:navigate class="btn btn-outline-secondary btn-sm flex-1 sm:flex-none justify-center">
                ...
                @if($can_edit)
                <a href="{{ route('admin.settings.catalog-products.brands.edit', $brand) }}" wire:navigate class="btn btn-primary btn-sm flex-1 sm:flex-none justify-center">
                    Editar
                </a>
                @endif
                ...
                <button type="button" wire:click="deleteRecord"
                    @disabled(! $can_delete)
                    title="{{ $is_general_readonly ? 'Marca general del sistema: no se puede eliminar' : ($has_equipment_usage ? 'No se puede eliminar: también está asociada a equipos' : ($can_delete ? 'Eliminar marca' : 'No se puede eliminar: tiene productos asociados')) }}"
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
            ...
            <dl class="divide-y divide-slate-100 px-5 py-2">
                ...
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                    <dt class="text-xs font-medium text-slate-500">Uso</dt>
                    <dd class="sm:col-span-2">
                        <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 ring-1 ring-amber-600/20">Artículos / ítems</span>
                    </dd>
                </div>
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
            <div class="border-b border-slate-100 bg-slate-50/80 px-5 py-4">
                <h2 class="font-semibold text-slate-800">Categorías asociadas</h2>
            </div>
            <div class="px-5 py-5">
                @if($brand->productCategories->isNotEmpty())
                <ul class="flex flex-wrap gap-2">
                    @foreach($brand->productCategories as $category)
                    <li class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700 ring-1 ring-slate-500/20">
                        {{ $category->name }}
                    </li>
                    @endforeach
                </ul>
                @else
                <p class="text-sm text-slate-500">Sin categorías asociadas.</p>
                @endif
                ...
                @if($has_equipment_usage)
                <p class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-600">
                    Esta marca también está registrada para equipos. La gestión de equipos se realiza desde Configuración → Equipos.
                </p>
                ...
                <p class="mt-4 rounded-xl border border-amber-100 bg-amber-50 px-3.5 py-2.5 text-xs text-amber-800">
                    Esta marca está en uso y no puede eliminarse hasta que no tenga productos asociados.
                </p>
                @else
