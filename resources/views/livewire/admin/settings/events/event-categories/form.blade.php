        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">
                    {{ $form->isEditing() ? 'Editar categoría de evento' : 'Nueva categoría de evento' }}
                </h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Define el nombre, descripción y periodicidad de esta categoría.</p>
            </div>
            <a href="{{ route('admin.settings.events.event-categories.index') }}" wire:navigate class="btn btn-outline-secondary btn-sm w-full justify-center sm:w-auto">Volver</a>
        </div>
    </header>
    ...
    <form wire:submit="save" class="space-y-6">
        <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm">
            <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-4 sm:px-5">
                <h2 class="font-semibold text-slate-800">Información de la categoría</h2>
            </div>
            <div class="space-y-5 p-4 sm:p-5">
                <p class="rounded-xl border px-3.5 py-2.5 text-xs {{ $is_super_admin ? 'border-indigo-100 bg-indigo-50 text-indigo-800' : 'border-slate-200 bg-slate-50 text-slate-600' }}">
                    @if($is_super_admin)
                        Esta categoría será <strong>general</strong>, sin negocio asociado.
                    @else
                        Esta categoría quedará asociada a tu iglesia.
                    @endif
                </p>
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input wire:model="form.name" type="text" placeholder="Ej. Culto dominical"
                ...
                <div>
                    ...
                    <textarea wire:model="form.description" rows="4" placeholder="Describe el propósito de esta categoría"
        ...
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <a href="{{ route('admin.settings.events.event-categories.index') }}" wire:navigate class="btn btn-outline-secondary w-full justify-center sm:w-auto">Cancelar</a>
