<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="text-slate-300">/</span>
        <span class="font-semibold text-slate-900">Tipos de negocio</span>
    </nav>
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Tipos de negocio</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Clasificación detallada vinculada a cada tipo de organización (Taller, Iglesia, Centro Educativo).</p>
            </div>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row">
        ...
        <select wire:model.live="filter_organization_type"
            class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm sm:w-auto">
            <option value="">Todos los tipos de organización</option>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        ...
        @if($business_types->isEmpty())
            <div class="px-4 py-16 text-center sm:px-5">
                <p class="text-sm font-medium text-slate-900">No hay tipos de negocio</p>
            </div>
        @else
                ...
                <table class="w-full min-w-[720px] text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                            ...
                            <th class="hidden px-3 py-3 text-left md:table-cell sm:px-5">Tipo de organización</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($business_types as $type)
                            <tr class="transition hover:bg-slate-50/80" wire:key="business-type-{{ $type->id }}">
                                <td class="px-3 py-4 sm:px-5">
                                    <p class="font-medium text-slate-900">{{ $type->name }}</p>
                                    <p class="mt-0.5 text-xs text-indigo-600 md:hidden">{{ $type->organization_type?->name ?? '—' }}</p>
                                </td>
                                <td class="hidden px-3 py-4 md:table-cell sm:px-5">
                                    <span class="rounded-lg bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700">{{ $type->organization_type?->name ?? '—' }}</span>
                                </td>
            ...
            @if($business_types->hasPages())
                <div class="border-t border-slate-100 px-4 py-4 sm:px-5">{{ $business_types->links() }}</div>
            @endif
        @endif
    </section>
    ...
    <x-ui.modal centered maxWidth="md">
        ...
        <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6">
            <h3 class="text-base font-semibold text-slate-900">{{ $form->isEditing() ? 'Editar tipo de negocio' : 'Nuevo tipo de negocio' }}</h3>
