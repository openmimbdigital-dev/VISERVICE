<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="text-slate-300">/</span>
        <span class="font-semibold text-slate-900">Métodos de pago</span>
    </nav>
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Métodos de pago</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Formas de pago del negocio para cotizaciones. Los métodos generales aplican a todos los negocios y solo el superAdmin puede modificarlos.</p>
            </div>
            @can('business_payment_methods.create')
            <x-ui.create-button wire:click="openCreate" class="w-full justify-center sm:w-auto">Nuevo método</x-ui.create-button>
            @endcan
        </div>
    </header>
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-2xl border border-slate-200/90 bg-white px-4 py-4 shadow-sm ring-1 ring-slate-900/[0.035]">
            <p class="text-xs font-medium text-slate-500">Predeterminados</p>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-4 sm:px-5">
            <h2 class="font-semibold text-slate-800">Listado de métodos</h2>
        </div>
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:admin.businesses.payment-methods.datatable-business-payment-methods :key="'payment-methods-datatable'" />
        </div>
    </section>
    ...
    <x-ui.modal centered maxWidth="md">
        ...
        <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6">
            <h3 class="text-base font-semibold text-slate-900">{{ $form->isEditing() ? 'Editar método' : 'Nuevo método' }}</h3>
        ...
        <form wire:submit="save" class="flex min-h-0 flex-1 flex-col">
            <div class="flex-1 space-y-4 overflow-y-auto px-4 py-5 sm:px-6">
                ...
                <label class="flex items-center gap-3 rounded-xl border border-indigo-100 bg-indigo-50 px-3.5 py-2.5 text-sm text-indigo-900">
                    ...
                    <span>Método <strong>general</strong> (aplica a todos los negocios)</span>
                </label>
                ...
                <p class="rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-600">
                    El método quedará asociado a tu negocio.
                </p>
                @endif

                <div>
                    <label class="mb-1.5 block text-xs font-medium text-slate-700">Nombre <span class="text-rose-500">*</span></label>
                    <input wire:model="form.name" type="text" placeholder="Ej. Transferencia bancaria"
                ...
                <div>
                    ...
                    <input wire:model="form.sort_order" type="number" min="0" max="9999"
                        class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm @error('form.sort_order') border-rose-400 bg-rose-50 @enderror">
            ...
            <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 px-4 py-4 sm:flex-row sm:justify-end sm:gap-3 sm:px-6">
                ...
                <button type="submit" wire:loading.attr="disabled" class="inline-flex w-full items-center justify-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white sm:w-auto">
                    {{ $form->isEditing() ? 'Guardar cambios' : 'Crear método' }}
                </button>
            </div>
        </form>
    </x-ui.modal>
