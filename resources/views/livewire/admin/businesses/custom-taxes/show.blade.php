<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <div class="mt-2 flex flex-wrap items-center gap-3">
                    <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ $custom_tax->name }}</h1>
                    @if($custom_tax->general)
            ...
            <div class="flex w-full shrink-0 flex-wrap gap-2 sm:w-auto">
                ...
                <button type="button" disabled title="Impuesto general del sistema: no editable" class="btn btn-primary btn-sm flex-1 sm:flex-none justify-center disabled:opacity-50">Editar</button>
                @endif
                @endcan
                ...
                @elseif($is_general_readonly)
                <button type="button" disabled title="Impuesto general del sistema: no se puede eliminar" class="btn btn-danger btn-sm flex-1 sm:flex-none justify-center disabled:opacity-50">Eliminar</button>
                @endif
                @endcan
            </div>
        </div>
    ...
    <p class="mb-6 rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm text-indigo-800">
        Impuesto general del sistema. Los negocios pueden consultarlo pero no editarlo ni eliminarlo.
    </p>
    @endif
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
            ...
            <dl class="divide-y divide-slate-100 px-5 py-2">
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                    ...
                    <dd class="text-sm text-slate-900 sm:col-span-2">{{ $custom_tax->general ? 'Sí — aplica a todos los negocios' : 'No' }}</dd>
                </div>
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
