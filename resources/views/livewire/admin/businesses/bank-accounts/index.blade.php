<div class="relative mx-auto w-full max-w-[90rem] p-4 sm:p-6">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="text-slate-300">/</span>
        <span class="font-semibold text-slate-900">Datos bancarios</span>
    </nav>
    ...
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-indigo-600 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Datos bancarios</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">Cuentas bancarias del negocio para mostrar en cotizaciones y documentos.</p>
            </div>
            @can('business_bank_accounts.create')
            <x-ui.create-button wire:click="openCreate" class="w-full justify-center sm:w-auto">Nueva cuenta</x-ui.create-button>
            @endcan
        </div>
    </header>
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div class="rounded-2xl border border-slate-200/90 bg-white px-4 py-4 shadow-sm ring-1 ring-slate-900/[0.035]">
            <p class="text-xs font-medium text-slate-500">Predeterminadas</p>
            <p class="mt-1 text-2xl font-bold text-indigo-700">{{ $stats['default'] }}</p>
        </div>
    </div>

    <section class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/[0.035]">
        <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-4 sm:px-5">
            <h2 class="font-semibold text-slate-800">Listado de cuentas</h2>
        </div>
        <div class="overflow-x-auto p-3 sm:p-4">
            <livewire:admin.businesses.bank-accounts.datatable-business-bank-accounts :key="'bank-accounts-datatable'" />
        </div>
    </section>
    ...
    <x-ui.modal centered maxWidth="xl">
        ...
        <div class="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-6">
            <h3 class="text-base font-semibold text-slate-900">{{ $form->isEditing() ? 'Editar cuenta bancaria' : 'Nueva cuenta bancaria' }}</h3>
        ...
        <form wire:submit="save" class="flex min-h-0 flex-1 flex-col">
            <div class="flex-1 space-y-4 overflow-y-auto px-4 py-5 sm:px-6">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-xs font-medium text-slate-700">Banco (catálogo)</label>
                        <select wire:model.live="form.bank_id" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm">
                    ...
                    <div>
                        <label class="mb-1.5 block text-xs font-medium text-slate-700">Nombre del banco <span class="text-rose-500">*</span></label>
                        <input wire:model="form.bank_name" type="text" placeholder="Ej. Bancolombia"
                            class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm @error('form.bank_name') border-rose-400 bg-rose-50 @enderror">
                    ...
                    <div>
                        ...
                        <input wire:model="form.account_number" type="text" placeholder="Ej. 123 456 789 01"
                    ...
                    <div>
                        ...
                        <input wire:model="form.account_holder" type="text" placeholder="Ej. Cerviteca S.A.S."
                    ...
                    <div>
                        ...
                        <input wire:model="form.document_number" type="text" placeholder="Ej. 900.123.456-7"
                ...
                <label class="flex items-center gap-3 text-sm text-slate-700">
                    <input type="checkbox" wire:model="form.is_default" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                    <span>Cuenta predeterminada para documentos</span>
                </label>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm {{ $form->active ? 'font-medium text-emerald-700' : 'text-slate-500' }}">{{ $form->active ? 'Activa' : 'Inactiva' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 px-4 py-4 sm:flex-row sm:justify-end sm:gap-3 sm:px-6">
                ...
                <button type="submit" wire:loading.attr="disabled" class="inline-flex w-full items-center justify-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white sm:w-auto">
                    {{ $form->isEditing() ? 'Guardar cambios' : 'Crear cuenta' }}
                </button>
            </div>
        </form>
    </x-ui.modal>
