...
<body class="bg-slate-100 text-slate-900 antialiased min-h-screen">
    ...
    >
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen">
            ...
            {{-- Suplantación en curso: siempre visible, con la salida a mano --}}
            @if(\App\Support\Impersonation::isActive())
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-amber-300 bg-amber-100 px-4 py-2.5 lg:px-6">
                <p class="flex items-center gap-2 text-sm text-amber-900">
                    ...
                    <span class="text-amber-700">· {{ auth()->user()->business->name }}</span>
                    @endif
                </p>
                <form method="POST" action="{{ route('impersonation.stop') }}">
                    @csrf
                    <button type="submit"
                        class="inline-flex items-center gap-1.5 rounded-lg bg-amber-900 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-amber-800">
